...
def depositar(
    valor_depositado: float, conta: float, extrato: list, transacoes_realizas: int
) -> tuple[float, int]:
    if transacoes_realizas == TRANSACOES_DIARIO:
        print(
            f"Você atingiu o número máximo de transações diáras: {TRANSACOES_DIARIO} movimentações em {DATA_ATUAL.strftime('%d/%m/%Y às %H:%M')}"
        )
        ...
        return deposito, transacoes_realizas  # Deposito Realizado
...
def sacar(
    ...
    extrato: list,
    transacoes_realizas: int,
) -> tuple[float, int, int]:
    saque = conta - valor_solicitado

    if transacoes_realizas == TRANSACOES_DIARIO:
        print(
            f"Você atingiu o número máximo de transações diárias {TRANSACOES_DIARIO} movimentações em {DATA_ATUAL.strftime('%d/%m/%Y às %H:%M')}"
        )
        ...
        saques_realizados += 1
        transacoes_realizas += 1

        if extrato[0] == "zero":
            extrato[0] = f"Saque de R${valor_solicitado:.2f} realizado"
        else:
            extrato.append(f"Saque de R${valor_solicitado:.2f} realizado")
        return saque, saques_realizados, transacoes_realizas  # Saque Realizado

    return conta, saques_realizados, transacoes_realizas
...
def criar_usuario(usuarios):
    ...
    endereco = input(
        "Informe o endereço (logradouro, numero - bairro - cidade/sigla do Estado):    >>> "
    )
    usuarios.append(
        {
            "nome": nome,
...
while True:
    ...
    if escolha == 1:  # Depósito
        ...
        try:
            ...
            conta, transacoes_realizas = depositar(
                valor_digitado, conta, extrato, transacoes_realizas
            )  # Atualiza transacoes_realizas
        except ValueError:
        ...
        try:
            ...
            conta, saques_realizados = sacar(
                valor_digitado, conta, saques_realizados, extrato, transacoes_realizas
            )  # DESAFIO 2 -> KEYWORD ONLY
        except ValueError:
            print("Valor inválido. Por favor, digite um número.")
        menu_opcao()
        ...
        exibir_extrato(
            conta, extrato
        )  # Passa o saldo e o extrato para a função / DESAFIO 2 -> HIBRIDO
        menu_opcao()
